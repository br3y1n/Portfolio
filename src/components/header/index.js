...
const Header = () => {
    ...
    const
        ...
        [rotateLogo, setRotateLogo] = useState(false),
        ...
        _evaluateLink = useCallback(link => {
            ...
            switch (itemMethod) {
                case 'download':
                    ...
                    return file
                        ? <a key={id} href={file.node.file.localFile.publicURL} download={file.node.fileName} onClick={() => { setShowExpand(prevExpand => !prevExpand) }}>{itemName}</a>
                        : undefined

                case 'goPage':
                default:
                    return <Link to={`/${itemTarget}`} key={id} className={itemTarget === currentPage ? 'active' : ''} onClick={() => { setShowExpand(prevExpand => !prevExpand) }}>{itemName}</Link>
            }
    ...
    return (
        ...
        <header className={`noselect ${show ? 'active' : ''} ${showExpand ? 'expand' : ''} `}>
            <div className={`burger ${animate ? 'animate-logo' : ''}`} onClick={() => { setShowExpand(_toggleExpand) }}>
                {!showExpand && <div className={`logo-image  ${rotateLogo ? 'rotate' : 'return'}`}>
